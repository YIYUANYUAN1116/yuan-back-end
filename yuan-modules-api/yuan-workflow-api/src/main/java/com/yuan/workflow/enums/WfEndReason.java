...
public enum WfEndReason implements BaseEnum {
    ...
    public static WfEndReason getByCode(String code) {
        for (WfEndReason wfEndReason : WfEndReason.values()) {
            if (wfEndReason.code.equals(code)) {
                return wfEndReason;
            }
        }
        throw new IllegalArgumentException("[WfEndReason] unknown code " + code);
    }
}